...
```sh
omarchy plugin validate ~/.config/omarchy/plugins/io.github.theneovimmer.omarec
omarchy plugin enable io.github.theneovimmer.omarec right
```

To remove it completely, turn off the overlay and let Omarchy remove the plugin:

```sh
~/.config/omarchy/plugins/io.github.theneovimmer.omarec/bin/omarec off
omarchy plugin remove io.github.theneovimmer.omarec --yes
```

There are no system configuration changes to undo. Removing the plugin leaves your
saved camera settings behind in `~/.local/state/omarec/` — delete that folder too
for a complete wipe.

## Requirements

- [Omarchy](https://omarchy.org) Quattro (Hyprland + Quickshell shell).
- A capture-capable webcam (`/dev/video*`, checked with `v4l2-ctl --list-devices`).
- System tools, all checked by `omarec check`: `mpv` (renders the overlay),
  `v4l2-ctl` (camera detection and capture options), `jq` and `hyprctl`
  (overlay state and window rules). Optional: `omarchy-notification-send` and
  `omarchy-menu-select` (notifications and the camera picker degrade gracefully
  without them).
- Permissions: runs entirely as your user — no root, no daemons, no system
  services. It reads the camera device, drives `mpv` and `hyprctl`, and writes
  only its own state under `~/.local/state/omarec/`. It never records, never
  touches the network, and never modifies Omarchy or Hyprland configuration.
...
`~/.local/state/omarec/omarec.conf` (under `$XDG_STATE_HOME`), seeded once from the
shipped `omarec.conf` template — deliberately outside the plugin folder, because the
shell hot-reloads the plugin on any change there and would abort in-flight restarts.
...
owning all processes/timers/IPC — the `io.github.theneovimmer.omarec` IPC target: `on`, `off`, `toggle`,
...
Delete the `io.github.theneovimmer.omarec` folder and remove the
`io.github.theneovimmer.omarec` entry from `shell.json`, then delete
`~/.local/state/omarec/` to also drop the remembered camera settings.
